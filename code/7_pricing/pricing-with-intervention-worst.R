...
assumption_lapse <- 0.01 * 1.2
assumption_reserve_T20_S <- 0.35
assumption_reserve_T20_NS <- 0.06

assumption_smoking_cost <- 2290.3 * 1.2 # not in premium, 1yr free, 3yr 50%
assumption_smoking_ratio <- 0.5
assumption_cancer_cost <- 348.6 * 1.2 # in premium
...
#### WORST CASE
i_discount <- i_discount |> mutate(rate = rate * 0.8)
...
techprem_perdb <- expand.grid(issue_year = min(PH_existing$Issue.year):max(PH_future$Issue.year),
  ...
  write_csv("data/techprem_perdb_intervention_worst.csv")
### 19 minutes
...
cancerprem_percost <- expand.grid(issue_year = min(PH_existing$Issue.year):max(PH_future$Issue.year),
  ...
  write_csv("data/cancerprem_percost_intervention_worst.csv")
### 10 minutes
...
for (i in c("LT_T20_F_NS", "LT_T20_F_S34", "LT_T20_F_S44", "LT_T20_F_S54", "LT_T20_F_S64",
  ...
  mdt |> write_csv(paste0("data/mdt", "_", name, "_intervention_worst.csv"))
}
...
for (i in min(PH_future$Issue.year):max(PH_future$Issue.year)) {
  ...
  scored_existing |> 
    ...
    write_csv(paste0("data/", year, "ledger-with-intervention-worst.csv"))
}
